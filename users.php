...
if(isset($_GET['smar_nav']) && $_GET['smar_nav'] == 'true') {
	...
	switch($subpage) {
		...
		default:
			...
			if(!$fullPermission && $userid != $_SESSION['loginID']) {
				$SMAR_MESSAGES['error'][] = 'Access denied.<br />You don\'t have permission to change other users password.';
				smar_print_messages($SMAR_MESSAGES); unset($SMAR_MESSAGES);
			} else {
